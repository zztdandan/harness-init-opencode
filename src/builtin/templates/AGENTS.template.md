...
- `scripts/shell_source.sh` 与 `scripts/session_env.json` 仅影响 bash 前置行为，不在会话内显式执行。
- 在没有 `harness-agent-env` 技能管理时，不允许调整 `scripts/shell_source.sh` 与 `scripts/session_env.json`。
