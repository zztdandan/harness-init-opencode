...
本文档定义 `check-agent-env.sh` 的主输出格式，适用于基础环境（Python/Javascripts/Shell）以及 `session_env.json` 变量回显。

## 生效范围

- 所有项目通用。
- 当前是主规范，包含 Python/Javascripts/Shell 三种语言环境的检查输出，以及 `scripts/session_env.json` 中变量清单输出。
...
4. `command` 表示当前目录可调用命令：
   ...
   - 不可直接调用时写绝对路径
5. `command` 不带参数。
6. 必须输出 `session_env` 段，打印 `scripts/session_env.json` 中全部变量。
...
[session_env.vars]
# 如实反应读取 scripts/session_env.json 后的全部变量键值对
...
- 不得遗漏 `session_env.vars` 中任一已设定变量。
