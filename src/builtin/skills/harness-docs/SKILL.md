...
6. 管理阶段输入优先级：
   - P0（权威事实）：根目录 `AGENTS.md`、`scripts/check-agent-env.sh`、`scripts/init-agent-env.sh`、当前仓库 git 结构
   - P1（参考约束）：模板文件（若可见）
...
- 输入（管理）：根目录 `AGENTS.md`（必读）+ `scripts/check-agent-env.sh` + `scripts/init-agent-env.sh` + docs/gittree 事实；`.tmp/` 仅在存在时参考
- 输出：根目录 `AGENTS.md`
