...
5. 若需要完整示例代码，应优先阅读配套文档 `docs/OPENCODE_PLUGIN_DEVELOPMENT_REFERENCE_EXAMPLES.md`。
...
1. 先照 `docs/OPENCODE_PLUGIN_DEVELOPMENT_REFERENCE_EXAMPLES.md` 创建最小骨架代码；
...
2. 目标 `agent` 存在；
3. 目标 `skill` 存在；
4. 若插件注册了 tool，则相应 tool 元数据存在；
5. 路径落在预期 `dist/` 输出下；
6. 命令执行 cwd 是测试 workspace。
