...
日期：2026-04-16  
用途：作为 `docs/OPENCODE_PLUGIN_DEVELOPMENT_GUIDE.md` 的配套“可直接迁移”示例文档
...
export function createGenericPlugin(paths: RuntimePaths): Plugin {
  return {
    ...
    async config(inputConfig) {
      return applyConfigMerge(inputConfig, {
...
describe("opencode dist load", () => {
  it("loads plugin from dist", async () => {
    ...
    const result = await $`opencode debug config --json`.cwd(ws).quiet()
...
- Adds reusable plugin capabilities for component development workflows.
- Injects agent prompts, built-in skills, and optional tools.

## Quick Start

1. Install dependencies: `bun install`
2. Build plugin: `bun run build`
3. Mount in `opencode.json`:

```json
{
  "plugin": [
    "file:///ABSOLUTE/PATH/TO/dist/plugin.js"
  ]
}
```
...
- Config is merge-safe and idempotent.
- Builtin assets are loaded from `dist/builtin`.
- Tool writes are restricted to workspace-relative paths.

## Verify

- `bun test`
- `opencode debug config --json`
````

这个 README 模板可直接迁移，只需替换插件能力描述和命令。
